...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold mb-0 text-dark">Gestión de Portales de Clientes</h2>
            <p class="text-muted small mb-0">Copia el enlace directo para enviar a tus clientes</p>
        </div>
    </div>

    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-header bg-white p-4 border-0">
            <form action="{{ route('empresa.clientes.portal_list') }}" method="GET" class="row g-2">
                <div class="col-md-10">
                    <div class="input-group">
                        <span class="input-group-text bg-light border-0"><i class="fas fa-search"></i></span>
                        <input type="text" name="q" class="form-control bg-light border-0" placeholder="Buscar por nombre o documento..." value="{{ request('q') }}">
                    </div>
                </div>
                <div class="col-md-2">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="ps-4">
                            ...
                            <div class="x-small text-muted">{{ $c->phone ?: 'Sin teléfono' }}</div>
                        </td>
                        <td>{{ $c->document ?: '-' }}</td>
                        <td>{{ $c->email ?: '-' }}</td>
                        <td class="text-end pe-4">
                            <button class="btn btn-sm btn-primary rounded-pill px-4 fw-bold shadow-sm btn-portal" 
                                    onclick="copyPortalLink(this, {{ $c->id }}, 'cliente')">
